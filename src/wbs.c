...
void bs_rec(double *x, int *n, int s, int e, int *len, double *res,double *iplus, double *iminus, double *ipres,int *ipargmax, double *ipmax, double minth, int scale){
	...
	*len = e-s+1;
	...
	if(*len>1){
		...
		bs_rec(x, n, s, cptcand, len, res,iplus, iminus,ipres,ipargmax, ipmax, minth, scale+1);
		bs_rec(x, n, cptcand+1, e, len, res,iplus, iminus,ipres,ipargmax, ipmax, minth, scale+1);
...
void bs_rec_wrapper(double *x, int *n, double *res){
	...
	bs_rec(x, n, 1, *n, len, res,iplus, iminus,ipres,ipargmax, ipmax, -1.0, 1);
...
void wbs_int_rec(double *x, int *n, int s, int e, int *len, double *res, double *iplus, double *iminus, double *ipres, int *ipargmax, double *ipmax, double *wbsres, int *index, int indexn, int *M, double minth, int scale){
	*len = e-s+1;
	...
	if(*len>1){
		if(indexn > 0){
			...
			if(indexnl){
				...
				wbs_int_rec(x, n, s, cptcand, len, res, iplus, iminus, ipres, ipargmax, ipmax, wbsres, indexl, indexnl, M,minth, scale+1);
				Free(indexl);
			}else{
				Free(indexl);
			...
			if(indexnr){
				...
				wbs_int_rec(x, n, cptcand +1, e, len, res, iplus, iminus, ipres, ipargmax, ipmax, wbsres, indexr, indexnr, M,minth,scale+1);
				Free(indexr);
			}else{
				Free(indexr);
			...
			bs_rec(x, n, s, e, len, res,iplus, iminus,ipres,ipargmax, ipmax,minth, scale);
		}
	}
}
void wbs_int_rec_wrapper(double *x, int *n, double *res,  int *intervals, int *M){
	...
	wbs_int_rec(x, n, 1, *n, len, res, iplus, iminus, ipres, ipargmax, ipmax, wbsres, index, *M, M,-1.0,1);
...
void wbs_rec(double *x, int *n, int s, int e, int *len, double *res, double *wbsres, int *index, int indexn, int *M, int scale){
	...
	if(*len>1){
		if(indexn > 0){
			...
			if(indexnl){
				indexl = Realloc(indexl,indexnl,int);
				wbs_rec(x, n, s, cptcand, len, res, wbsres, indexl, indexnl, M, scale+1);
				Free(indexl);
			}
			if(indexnr){
				indexr = Realloc(indexr,indexnr,int);
				wbs_rec(x, n, cptcand +1, e, len, res,  wbsres, indexr, indexnr, M, scale+1);
				Free(indexr);
			}
...
void wbs_rec_wrapper(double *x, int *n, double *res,  int *intervals, int *M){
	...
	wbs_rec(x, n, 1, *n, len, res, wbsres, index, *M, M, 1);
